...
def lock():
    ...
    save(SHEET,sheet)
    (ROOT/'SHOPPING.md').write_text('# Audio-locked asset plan\n\nNo stock shopping or paid generation needed. Eight source/result pairs use actual engine captures and native vector previews. Godot/Claude/assignment instructions are code-native scenes. All 30 beats have renderable sources.\n\nMeasured runtime: %.3f seconds.\n'%sheet['metadata']['duration_s'])
...
def footage(only=None,force=False):
    ...
    for b in sheet['beats']:
        ...
        raw=probe(source);start=0 if b['gameplay_take']=='controls' else 1.1
        # The baseline beat includes replay. Later mechanism beats end at the
        # relevant result, before the recorded replay hides the finished map.
        # All boundaries come from finish-states.jsonl (complete 6.9667s;
        # replay 8.5s), not invented state or a slowed-down game.
        end={'B14':8.3,'B18':6.933333333333333,'B20':6.933333333333333}.get(bid,raw)
        played=end-start;total=b['actual_duration_s']
        assert total>=played,'Narration shorter than played action; never silently trim.'
        label=ROOT/'media'/f'{bid}-label.png';hold=ROOT/'media'/f'{bid}-hold.png'
        overlay(label,b['heading'],'Actual Godot output · scripted normal input · original starter')
        overlay(hold,b['heading'],'HELD FRAME for explanation · played action was not slowed down')
        if not target.exists() or force:
            print('FOOTAGE',bid,flush=True)
            run(['ffmpeg','-v','error','-y','-ss',str(start),'-t',str(played),'-i',source,'-i',label,'-i',hold,'-filter_complex_threads','2','-filter_complex',f"[0:v]fps=30,tpad=stop_mode=clone:stop_duration={total}[base];[base][1:v]overlay=0:0:enable='lt(t,{played})'[v1];[v1][2:v]overlay=0:0:enable='gte(t,{played})'[out]",'-map','[out]','-t',str(total),'-an','-c:v','libx264','-threads','4','-preset','fast','-crf','16','-pix_fmt','yuv420p',target])
        ...
        edits.append({'beat':bid,'source':str(source.relative_to(ROOT)),'source_sha256':sha(source),'source_start_s':start,'source_end_s':end,'normal_speed_interval_s':played,'tail_hold_s':total-played,'output_duration_s':total,'retimed':False,'output_sha256':sha(target)})
    save(SHEET,sheet)
...
def render(only=None,force=False):
    for p in ['player-art.png','player-collider.png']:shutil.copy2(ROOT/'media'/p,ROOT/'remotion/public'/p)
